...
interface Props { name?: string; eventTitle: string; eventDate: string; spaces: number; checkoutUrl: string }
...
const styles = {
  ...
  actionBox: { padding: '16px 20px', border: '1px solid rgba(237,234,90,0.5)', borderRadius: '6px', backgroundColor: 'rgba(237,234,90,0.08)', marginBottom: '24px' },
  actionLabel: { margin: '0 0 8px', fontFamily: 'Inter, Arial, sans-serif', fontSize: '13px', fontWeight: 600, color: '#edea5a', textTransform: 'uppercase' as const, letterSpacing: '0.08em' },
  actionText: { margin: '0 0 12px', fontFamily: 'Inter, Arial, sans-serif', fontSize: '14px', color: '#edea5a', lineHeight: '1.5' },
  actionLink: { color: '#edea5a', fontWeight: 600 },
...
export function PaymentLinkEmail({ name, eventTitle, eventDate, spaces, checkoutUrl }: Props) {
  return (
    <Html>
      <Head />
      <Preview>Complete your booking for {eventTitle}</Preview>
        ...
        <Section style={styles.outer}>
          <Container style={styles.card}>
            ...
            <Section style={styles.bodyCell}>
              <Text style={styles.h1}>Complete your booking.</Text>
              {name
                ? <Text style={styles.subhead}>Hi {name} — one step to secure your spot.</Text>
                : <Text style={styles.subhead}>One step to secure your spot.</Text>
              }

              <div style={styles.box}>
                <Text style={styles.boxLabel}>{eventTitle}</Text>
                <Text style={{ ...styles.row, margin: '0 0 6px' }}><strong>Date:</strong> {eventDate}</Text>
                <Text style={{ ...styles.row, margin: '0' }}><strong>Spaces:</strong> {spaces}</Text>
              </div>

              <div style={styles.actionBox}>
                <Text style={styles.actionLabel}>Action needed</Text>
                <Text style={styles.actionText}>Pay for your booking to confirm your spot.</Text>
                <Text style={{ ...styles.actionText, margin: '0' }}>
                  <Link href={checkoutUrl} style={styles.actionLink}>{checkoutUrl}</Link>
                </Text>
              </div>
            </Section>
            ...
            <Section style={styles.footer}>
              <Text style={styles.footerText}>
                You&apos;re receiving this because a booking was started for you at backyard-sauna.com.
              </Text>
            </Section>
